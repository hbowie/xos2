 ...
 * <p>
 * Copied and included in com.powersurgepub.xos package on August 7, 2004 by Herb Bowie.
 *
...
public class BrowserLauncher {
  ...
  /** the com.apple.eio.FileManager class */
  private static Class fileManagerClass;
	...
	/** The openURL method of com.apple.mrj.MRJFileUtils or com.epple.eio.FileManager */
	...
	/** JVM constant for MRJ 3.2 */
	private static final int MRJ_3_2 = 5;
  
  private static final int MRJ_UNKNOWN = 5;
  ...
  /**
   * A string showing decisions that were made about the operating environment. 
   */
  private static String envInfo;
	 ...
	 */
	static {
    StringBuilder envWork = new StringBuilder();
		loadedWithoutErrors = true;
		String osName = System.getProperty("os.name");
    envWork.append("OS Name = " + osName);
    if (debug) {
      System.out.println ("OS Name = " + osName);
    }
		if (osName.startsWith("Mac OS")) {
			String mrjVersion = System.getProperty("mrj.version");
      envWork.append(", MRJ Version = " + mrjVersion);
      if (debug) {
        System.out.println ("MRJ Version = " + mrjVersion);
      }
      int majorEnd = 3;
      if (mrjVersion.length() < majorEnd) {
        majorEnd = mrjVersion.length();
      }
			String majorMRJVersion = mrjVersion.substring(0, majorEnd);
			try {
				...
				if (version == 2) {
					...
					// Assume that all 3.2 and later versions of MRJ work the same.
					jvm = MRJ_3_2;
				} else {
					jvm = MRJ_UNKNOWN;
    ...
    envWork.append(", JVM Switch = " + String.valueOf(jvm));
    if (debug) {
      System.out.println("JVM switch = " + String.valueOf(jvm));
    }
		
		if (loadedWithoutErrors) {	// if we haven't hit any errors yet
			loadedWithoutErrors = loadClasses();
		}
    envInfo = envWork.toString();
	}
      ...
      */
      if (jvm == MRJ_3_2) {
				try {
					...
					openURL = fileManagerClass.getDeclaredMethod("openURL", new Class[] { String.class });
				} catch (ClassNotFoundException cnfe) {
					errorMessage = cnfe.getMessage();
					return false;
				} catch (NoSuchMethodException nsme) {
  ...
  public static boolean getLoadedWithoutErrors() {
    return loadedWithoutErrors;
  }
  
  public static int getJVMSwitch() {
    return jvm;
  }
	 ...
	 */
	public static void openURL(String inurl) throws IOException {
    // System.out.println ("BrowserLauncher openURL inurl " + inurl);
		if (! loadedWithoutErrors) {
			throw new IOException(envInfo + ", browser = " + browser.toString() 
          + " Exception in finding browser: " + errorMessage);
		}
		Object browser = locateBrowser();
		if (browser == null) {
			throw new IOException(envInfo + " Unable to locate browser: " + errorMessage);
		}
		
    if (debug) {
      System.out.println ("BrowserLauncher openURL jvm = " + String.valueOf(jvm));
    }
    String url = inurl;
    if (debug) {
    ...
    try {
      ...
      // System.out.println ("BrowserLauncher openURL urlObj " + urlObj.toString());
      // String protocol = urlObj.getProtocol();
      // System.out.println ("BrowserLauncher openURL protocol " + protocol);
      // String authority = urlObj.getAuthority();
      // System.out.println ("BrowserLauncher openURL authority " + authority);
      // String host = urlObj.getHost();
      // System.out.println ("BrowserLauncher openURL host " + host);
      // int port = urlObj.getPort();
      // System.out.println ("BrowserLauncher openURL port " + String.valueOf(port));
      // String path = urlObj.getPath();
      // System.out.println ("BrowserLauncher openURL path " + path);
      // String file = urlObj.getFile();
      // System.out.println ("BrowserLauncher openURL file " + file);
      // String ref = urlObj.getRef();
      // System.out.println ("BrowserLauncher openURL ref " + ref);
      // String userInfo = urlObj.getUserInfo();
      // System.out.println ("BrowserLauncher openURL userInfo " + userInfo);
      // String query = urlObj.getQuery();
      // System.out.println ("BrowserLauncher openURL query " + query);
      
      /*
      StringBuffer urlEncoded = new StringBuffer();
      
      urlEncoded.append (protocol);
      urlEncoded.append (":");
      if (protocol.equalsIgnoreCase ("http")) {
        urlEncoded.append ("//");
      }
      urlEncoded.append (host);
      */
      
      /*
      if (path.length() > 0) {
        urlEncoded.append (URLEncoder.encode (path, scheme));
      }
      if (query != null) {
        urlEncoded.append (URLEncoder.encode (query, scheme));
      }
      */
      /*
      if (file.length() > 0) {
        urlEncoded.append (URLEncoder.encode (file, scheme));
      }
      if (ref != null) {
        urlEncoded.append (URLEncoder.encode (ref, scheme));
      }
      if (userInfo != null) {
        urlEncoded.append (URLEncoder.encode (userInfo, scheme));
      }
      
      System.out.println ("BrowserLauncher openURL encoded url " + urlEncoded);
      url = urlEncoded.toString();
      */
    ...
    //   System.out.println ("Malformed URL");
      ...
      System.out.println ("BrowserLauncher openURL unsupported encoding");
    }
    
    // System.out.println ("BrowserLauncher openURL url " + url);
		switch (jvm) {
			case MRJ_2_0:
        /*
				Object aeDesc = null;
        ...
        */
        ...
        /*
          ...
          // System.out.println ("BrowserLauncher openURL MRJ 3.1 successful");
				} catch (InvocationTargetException ite) {
         ...
         */
			case MRJ_3_2:
				try {
					openURL.invoke(null, new Object[] { url });
          if (debug) {
            System.out.println ("BrowserLauncher openURL MRJ 3.2 successful");
          }
				} catch (InvocationTargetException ite) {
					throw new IOException("InvocationTargetException while calling openURL: " + ite.getMessage());
				} catch (IllegalAccessException iae) {
		  ...
		  case WINDOWS_9x:
		    	...
		    	// characters to work.
        if (debug) {
          System.out.println("Windows Runtime exec = " + browser.toString());
          System.out.println("                       " + FIRST_WINDOWS_PARAMETER);
          System.out.println("                       " + SECOND_WINDOWS_PARAMETER);
          System.out.println("                       " + THIRD_WINDOWS_PARAMETER);
        ...
        if (debug) {
          System.out.println("UNIX Runtime exec = " + browser.toString());
          System.out.println("                    " + NETSCAPE_REMOTE_PARAMETER);
          System.out.println("                    " + NETSCAPE_OPEN_PARAMETER_START +
													                            url +
        ...
        if (debug) {
          System.out.println("Other Runtime exec = " + browser.toString());
          System.out.println("                     " + url);
        }
				Runtime.getRuntime().exec(new String[] { (String) browser, url });
				break;
